...
MAX_ANGULAR_VEL = 3.5
MAX_LINEAR_VEL  = 9.5
CENTER_THRESHOLD = 0.10   # fraction of half-width: inside this → linear mode

SEARCH_VEL       = 0.6    # angular speed while searching
...
def compute_motor_command(box_xyxy, frame_width: int, frame_height: int) -> MotorCommand:
    ...
    if abs(offset) > CENTER_THRESHOLD:
        ...
        return MotorCommand(linear_vel=linear_vel, angular_vel=0.0)
...
class SearchSweep:
    """
    Step-and-wait search: turn SEARCH_VEL for SEARCH_TURN_SECS,
    pause for SEARCH_WAIT_SECS, repeat — reversing direction every 4 steps.
    """

    def __init__(self):
        self._phase       = "wait"   # "turn" | "wait"
        ...
        if self._phase == "turn":
            if elapsed >= SEARCH_TURN_SECS:
                ...
                if self._steps % 4 == 0:
                    self._direction *= -1
            return MotorCommand(linear_vel=0.0,
                                angular_vel=self._direction * SEARCH_VEL)
        else:
            if elapsed >= SEARCH_WAIT_SECS:
                self._phase       = "turn"
                self._phase_start = now
            return MotorCommand(linear_vel=0.0, angular_vel=0.0)
